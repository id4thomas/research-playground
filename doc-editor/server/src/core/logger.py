...
jarvis-patent-api 의 패턴을 차용:
- console 포맷터(ANSI 컬러) / json 포맷터를 settings로 선택
- pathname:lineno - funcName 까지 표시해 디버깅 편의 ↑
"""
import json
import logging
...
class ConsoleFormatter(logging.Formatter):
    ...
    def _format_general_record(self, record: logging.LogRecord) -> str:
        ...
        # msg는 이미 보간된 문자열로 교체하고 args를 비워, super().format() 안의
        # record.getMessage()가 다시 `%`-치환을 시도하지 않도록 한다.
        record.msg = f"{log_color}{rendered_msg}{self._RESET}"
        record.args = None
...
def get_logger(
    ...
    if format == "console":
        formatter: logging.Formatter = ConsoleFormatter(datefmt=DATE_FORMAT)
    elif format == "json":
    ...
    # 동일 logger에 핸들러가 누적되지 않도록 한 번만 부착
